...
export const AlgorithmLandscape: FC<AlgorithmLandscapeProps> = ({ variant = 'catalog' }) => {
  ...
  return (
    <div className={`algorithm-landscape ${isBinarySearch ? 'algorithm-landscape--binary' : ''}`}>
      ...
      <div className="landscape-card landscape-card--left">
        <div className="flex items-start justify-between gap-5">
          <div>
            <p className="landscape-card-label">{isBinarySearch ? 'Complexidade' : 'Aprendizado'}</p>
            <p className="mt-2 text-sm font-medium text-white">
              {isBinarySearch ? 'O(log n)' : 'Visual e prático'}
            </p>
          </div>
      ...
      <div className="landscape-card landscape-card--right">
        <div className="flex items-start justify-between gap-5">
          <div>
            <p className="landscape-card-label">{isBinarySearch ? 'Valor procurado' : 'Trilha disponível'}</p>
            <p className="mt-2 flex items-center gap-2 text-lg font-semibold text-white">
              {isBinarySearch ? '23' : 'Busca Binária'}
              {!isBinarySearch && <CheckCircle2 size={14} className="text-violet-300" />}
            </p>
          </div>
      ...
      {isBinarySearch && (
        <div className="landscape-values" role="img" aria-label="Prévia visual da busca binária com o valor 23 destacado">
          {BINARY_VALUES.map((value) => (
            <span
              key={value}
              className={value === 23 ? 'landscape-value landscape-value--active' : 'landscape-value'}
            >
              {value}
            </span>
          ))}
